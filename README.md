...
3\. \*\*Load:\*\* Cleaned records are converted to JSON and pushed to a Supabase PostgreSQL table (`fact\_sales2026`). 
...
\* \*\*Trigger:\*\* Configured to run automatically via a CRON schedule (daily at 5:45 PM) or manually via `workflow\_dispatch`. \*(Note: Scheduled runs are currently disabled for this static portfolio showcase).\*
